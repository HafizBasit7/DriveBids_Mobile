  ...
  switch (action.type) {
    case "INSERT_DAMAGE": {
      ...
      return {
        ...state,
        carDamageReport: {
          damageReport: [...state.carDamageReport.damageReport, action.value],
        },
      };
    }
    case "SET_DRAFT": {
    ...
    case "REMOVE_FEATURE": {
      const filtered = state?.features[action?.section].filter(
    ...
    case "UPDATE_FEATURE": {
      if (state.features && state.features[action.section]) {
        return {
          ...
          features: {
            ...state.features,
            [action.section]: [...state.features[action.section], action.value],
          },
        };
      }
...
export default function CarContextProvider({ children }) {
  ...
  return (
    <CarContext.Provider
      value={{ carState, dispatch, draftSave, resetDraftState, carPostAd }}
    >
      {children}
    </CarContext.Provider>
  );
