...
import Interior from "../../../assets/tahirAssets/Interior2";
...
const Interior2 = () => {
  ...
  const handleCamera = async () => {
    ...
    if (!result.canceled) {
      ...
      setLoading(true);
      try {
        const imgUrl = await uploadImage(result.assets[0], `interior-${index}`);
        dispatch({
          type: "UPDATE_IMAGE",
          section: "interior",
          index: index,
  ...
  return (
    <View style={styles.container}>
      ...
      <SectionHeader title={"Step 2 of 5"} />
      <View style={{ gap: 20, justifySelf: "center" }}>
        <Text style={styles.text}>
          Take a picture of the speedometer display
        </Text>
        <TouchableOpacity
        ...
        >
          {(carState.images.interior || [])[index]?.url ? (
            <>
              <Image
                source={{ uri: (carState.images.interior || [])[index]?.url }}
                style={styles.image}
              />
